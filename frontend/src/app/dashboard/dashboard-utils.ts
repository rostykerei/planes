...
export class DashboardUtils {
  ...
  static updatePosition(f: MapFlight, deltaTime: number): MapFlight {
    ...
    let startLatRad = this.deg2rad(f.lat);
    let startLonRad = this.deg2rad(f.lon);

    let headingRad = this.deg2rad(f.heading);
  ...
  static distance(lat: number, lon: number, f: MapFlight): number {
    let dLat = this.deg2rad(f.lat - lat);
    let dLon = this.deg2rad(f.lon - lon);
