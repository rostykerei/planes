...
public interface StatisticsRepository extends JpaRepository<Aircraft, Integer> {
    ...
    @Query("SELECT NEW nl.rostykerei.planes.server.response.NameValue(a.type.type, COUNT(a)) " +
    ...
    @Query("SELECT NEW nl.rostykerei.planes.server.response.NameValue(al.code, COUNT(al)) " +
            "FROM Flight f " +
